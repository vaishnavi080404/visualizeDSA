...
const PROBLEMS = [
  {
    ...
    realWorld:
      "Used in database indexing to find the range of records that fall within specific timestamps or price ranges.",
...
<br><br>Return both indices as a pair. If no such index exists, return <strong>n</strong>.`,
    examples: [
      {
        input: "arr = [1, 2, 4, 4, 4, 5, 7], x = 4",
        output: "Lower: 2, Upper: 5",
        explain:
          "First 4 is at index 2. First element greater than 4 is 5 at index 5.",
      },
      {
        input: "arr = [1, 2, 3, 5], x = 4",
        output: "Lower: 3, Upper: 3",
        explain:
          "First element $\ge$ 4 is 5 (index 3). First element $>$ 4 is also 5.",
      },
    ],
    constraints: ["1 ≤ n ≤ 10⁶", "1 ≤ arr[i], x ≤ 10⁹", "Array is sorted."],
...
if (input.length > 2) {
...

  {
    ...
    realWorld:
      "Used in maintaining a sorted leaderboard where a new score needs to be inserted at the correct rank.",
...
int main() {
...

  {
    ...
    realWorld:
      "Used in computer graphics for distance calculations and normalizing vectors where precision is capped at pixels.",
...
<br><br>Do not use any built-in exponent functions like <code>pow(x, 0.5)</code> or <code>sqrt()</code>.`,
    examples: [
      { input: "x = 4", output: "2" },
      {
        input: "x = 8",
        output: "2",
        explain: "Square root of 8 is 2.828..., so return 2.",
      },
    ],
    constraints: ["0 ≤ x ≤ 2³¹ - 1"],
...
int main() {
...

  {
    ...
    pattern: "Binary Search Boundary",
    realWorld:
      "Used in text search to find the full span of a recurring word in a document.",
...
<br><br>If target is not found in the array, return <code>[-1, -1]</code>.`,
    ...
    constraints: ["0 ≤ n ≤ 10⁵", "-10⁹ ≤ arr[i], target ≤ 10⁹"],
...
vector<int> searchRange(vector<int>& nums, int target) {
...

  {
    ...
    realWorld:
      "Used in decentralized systems where a circular list of nodes is partitioned and shifted across different servers.",
...
Given the array after rotation and an integer <code>target</code>, return the index of <code>target</code> if it is in <code>nums</code>, or <code>-1</code> if it is not.`,
    ...
    constraints: ["1 ≤ n ≤ 5000", "-10⁴ ≤ nums[i], target ≤ 10⁴"],
...
int main() {
...

  {
    ...
    realWorld:
      "Handling data with duplicates is common in logs where multiple events happen at the same timestamp, and the log is rotated daily.",
...
int main() {
...

  {
    ...
    pattern: "Binary Search Rotated",
    realWorld:
      "Used in log systems that wrap around. Finding the minimum is equivalent to finding the oldest log entry in a circular buffer.",
...
console.log(findMin(input));`,
...

  {
    ...
    realWorld:
      "Used in analyzing wave signals or heart rate monitors to detect the exact moment a pulse reaches its peak intensity.",
...
<br><br>Find that index <code>i</code> (the peak).`,
    ...
    constraints: ["3 ≤ arr.length ≤ 10⁵", "0 ≤ arr[i] ≤ 10⁶"],
...
int main() {
...

  {
    ...
    pattern: "Binary Search on Answer",
    realWorld:
      "Used in cloud resource management: what is the minimum bandwidth required to process <code>N</code> tasks within <code>H</code> hours without exceeding capacity?",
    ...
    examples: [
      {
        ...
        output: "4",
        explain: "With speed 4, piles take [1, 2, 2, 3] hours = 8 hours total.",
      },
    ],
    constraints: [
      "1 ≤ piles.length ≤ 10⁴",
...
int minEatingSpeed(vector<int>& piles, int h) {
...

  {
    ...
    pattern: "Binary Search on Answer",
    realWorld:
      "Used in printer load balancing: how to distribute print jobs among <code>M</code> printers to ensure the printer with the most work has the least amount possible.",
    ...
    examples: [
      {
        ...
        output: "113",
        explain:
          "Student 1: [12, 34, 67], Student 2: [90]. Max is 113. (Other combinations result in higher maximums).",
      },
    ],
    constraints: ["1 ≤ N ≤ 10⁵", "1 ≤ M ≤ N", "1 ≤ A[i] ≤ 10⁶"],
...
int findPages(vector<int>& A, int n, int m) {
...

  {
    ...
    realWorld:
      "Used in retail store placement: finding the minimum distance between stores to ensure they don't cannibalize each other's foot traffic while being spaced out effectively.",
    ...
    examples: [
      {
        ...
        output: "3",
        explain: "Place cows at 1, 4, and 9. Minimum distance is 3.",
      },
    ],
    constraints: ["2 ≤ N ≤ 10⁵", "2 ≤ K ≤ N", "0 ≤ stalls[i] ≤ 10⁹"],
...
int aggressiveCows(vector<int>& stalls, int k) {
...

  {
    ...
    realWorld:
      "Used in network planning to add 'repeaters' along a highway such that the largest dead-zone (distance without signal) is minimized.",
...
double minMaxGasDist(vector<int>& st, int k) {
...

  {
    ...
    pattern: "Binary Search on Answer",
    realWorld:
      "Supply chain optimization: determining the minimum truck capacity required to deliver all inventory within a hard deadline.",
    ...
    examples: [
      {
        ...
        output: "15",
        explain:
          "Capacity 15: Day 1 (1..5), Day 2 (6,7), Day 3 (8), Day 4 (9), Day 5 (10).",
      },
    ],
    constraints: ["1 ≤ D ≤ weights.length ≤ 5 * 10⁴", "1 ≤ weights[i] ≤ 500"],
...
int shipWithinDays(vector<int>& weights, int days) {
...

  {
    ...
    realWorld:
      "Used in distributed systems to merge sorted logs from different servers and find the global median event timestamp.",
...
int main() {
...

  {
    ...
    pattern: "Binary Search on Answer",
    realWorld:
      "Essential in financial calculators for calculating Compound Annual Growth Rate (CAGR).",
...
int main() {
...

  {
    ...
    pattern: "Binary Search on Partition",
    realWorld:
      "Used in databases when fetching a specific record from a union of two massive sorted tables stored on different disks.",
...
int main() {
...

  {
    ...
    realWorld:
      "Used in topographic survey data to find local mountain peaks from a grid of height samples.",
    ...
    examples: [{ input: "mat = [[1,4],[3,2]]", output: "[0,1] or [1,0]" }],
...
int main() {
...

  {
    ...
    realWorld:
      "Finding the median statistic in a distributed database where each shard (row) stores sorted data.",
    ...
    examples: [{ input: "mat = [[1,3,5],[2,6,9],[3,6,9]]", output: "5" }],
    constraints: ["1 ≤ R, C ≤ 150", "1 ≤ mat[i][j] ≤ 2000"],
...
int median(vector<vector<int>> &matrix, int R, int C) {
...

  {
    ...
    realWorld:
      "Navigating a structured grid where both axes have increasing values, such as a specialized spreadsheet for financial risk tiers.",
