...
const TOPIC_FILES = {
  ...
  linkedlist: "linkedlist_problems.js",
  "linked-list": "linkedlist_problems.js",
  trees: "tree_problems.js",
  hashing: "hashing_problems.js",
  heap: "heap_problems.js",
  graphs: "graphs_problems.js",
};
...
// Solved problem IDs aren't globally unique on their own (every
// topic has its own "p1", "p2"...), so we prefix with the topic
// name before saving to the DB to keep them from colliding.
const solvedKeyFor = (id) => `${topicParam}:${id}`;

function loadSolvedFromServer() {
  solved = new Set(
    (window.__solvedProblems || [])
      .filter((s) => s.startsWith(`${topicParam}:`))
      .map((s) => s.split(":")[1]),
  );
}
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  // Load solved state for this topic — window.__solvedProblems
  // is populated by navbar.js's profile fetch, which may not have
  // resolved yet, so we load what's there now and refresh once
  // "progressReady" fires with the real data from the DB.
  loadSolvedFromServer();
...
function markSolved(id) {
  // 1. Mark the specific problem as solved for the sidebar UI
  solved.add(id);
  renderProblemList();

  // ...and persist it to the account in the DB, not the browser
  fetch(`${typeof BASE_URL !== "undefined" ? BASE_URL : "http://localhost:3001/api"}/user/solved`, {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
      Authorization: `Bearer ${localStorage.getItem("token")}`,
    },
    body: JSON.stringify({ problemId: solvedKeyFor(id) }),
  })
    .then((res) => res.json())
    .then((data) => {
      window.__solvedProblems = data.solvedProblems || window.__solvedProblems;
    })
    .catch((err) => console.error("Solved-problem sync failed:", err));

  // 2. Map URL topic names to the IDs used in topics.html

  const idMap = {
    // Sorting (Fixed Underscores)
  ...
  const targetTopicId = idMap[topicParam] || topicParam;
  console.log(`Marking topic ${targetTopicId} as Level 3 complete...`);

  // Update the in-memory copy so this page reflects it immediately;
  // the actual save-of-record is the MongoDB sync right below.
  window.__levelProgress = window.__levelProgress || {};
  window.__levelProgress[targetTopicId] = 3;

  // Sync to MongoDB
  fetch(`${typeof BASE_URL !== "undefined" ? BASE_URL : "http://localhost:3001/api"}/user/progress`, {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
      Authorization: `Bearer ${localStorage.getItem("token")}`,
