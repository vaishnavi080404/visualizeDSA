    ...
    function applyTheme(theme) {
        ...
        localStorage.setItem('dsa-theme', theme); if(window.syncThemeToServer)window.syncThemeToServer(theme);
    }
  ...
  document.querySelector('.nav-links').classList.toggle('mobile-open');
    ...
    function toggleOperation(id) {
        ...
        if (content) {
            ...
            if (icon) icon.textContent = content.classList.contains('show') ? '▲' : '▼';
        }
    }

    function switchCode(event, group, lang) {
        const tabs = event.target.parentElement;
        tabs.querySelectorAll('.lang-tab').forEach(tab => tab.classList.remove('active'));
        event.target.classList.add('active');
        document.querySelectorAll(`[data-group="${group}"]`).forEach(content => {
            if (content.classList.contains('code-lang-content')) content.classList.remove('active');
        });
...
const ALGO_KEY = document.title.split(' - ')[0].trim().toLowerCase().replace(/\s+/g, '_');
...
btnL2.onclick = () => {
    ...
    if (l2Unlocked) {
        ...
        startQuizBtn.onclick = () => window.location.href = `quiz.html?id=${ALGO_KEY}-l1`;
    }
};
...
btnL3.onclick = () => {
    ...
    if (l3Unlocked) {
        ...
        modal.querySelector('p').textContent = 'Pass the Level 1 Quiz to unlock Level 2 first before accessing Level 3.';
        modal.style.display = "flex";
        startQuizBtn.onclick = () => window.location.href = `quiz.html?id=${ALGO_KEY}-l1`;
    } else {
        // L2 unlocked but L3 not — prompt L2 quiz
        modal.querySelector('p').textContent = 'Pass the Level 2 Quiz to unlock Level 3 and complete your journey.';
        modal.style.display = "flex";
        startQuizBtn.onclick = () => window.location.href = `quiz.html?id=${ALGO_KEY}-l2`;
    }
};
...
cancelBtn.onclick = () => {
    ...
    modal.querySelector('p').textContent = `Pass the Level 1 Quiz to unlock Level 2 and continue your journey.`;
};
