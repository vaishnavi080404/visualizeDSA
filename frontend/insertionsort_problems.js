...
const PROBLEMS = [
  {
    ...
    realWorld:
      "Commonly used in hand-sorting playing cards or for sorting small chunks of data in hybrid algorithms like Timsort.",
    ...
    examples: [
      {
        ...
        output: "[5, 6, 11, 12, 13]",
        explain:
          "11 is inserted before 12, then 5 is moved to the front, then 6 is inserted after 5.",
      },
    ],
...
console.log(arr.join(' '));`,
  ...
  },
  {
    ...
    pattern: "Incremental Building",
    realWorld:
      "Used to display high scores or recent transactions where the largest/newest values should appear first.",
    description: `Modify the Insertion Sort algorithm to sort an array in <strong>descending order</strong> (Largest to Smallest).`,
    examples: [{ input: "arr = [5, 2, 9, 1]", output: "[9, 5, 2, 1]" }],
...
int main() {
  ...
  },
  {
    ...
    pattern: "Simulation",
    realWorld:
      "Debugging tool to visualize how the 'sorted' partition of an array grows during execution.",
...
int main() {
  ...
  },
  {
    ...
    realWorld:
      "Calculating energy/time cost of a sort on a specific CPU architecture by counting memory access comparisons.",
    ...
    examples: [
      {
        input: "arr = [3, 2, 1]",
        output: "3",
        explain: "2 vs 3 (match), then 1 vs 3 (match), then 1 vs 2 (match).",
      },
    ],
    constraints: ["1 ≤ n ≤ 500"],
...
int main() {
  ...
  },
  {
    ...
    realWorld:
      "In Flash Memory (SSD), 'writes' are expensive. Shifting elements counts as a write. This metric tells you how wear-intensive the sort is.",
    ...
    examples: [
      {
        ...
        output: "6",
        explain:
          "Shifts: [3->pos1], [3,4->pos1,2], [2,3,4->pos1,2,3]. Total 1+2+3 = 6.",
      },
    ],
    constraints: ["1 ≤ n ≤ 1000"],
...
int main() {
  ...
  },
  {
    ...
    realWorld:
      "Insertion sort is used in production as the final step of QuickSort/MergeSort because it is O(n) for nearly-sorted data.",
    ...
    examples: [
      {
        ...
        output: "4",
        explain:
          "For each element from index 1 to 4, we check the neighbor once, fail the condition, and move on.",
      },
    ],
    constraints: ["1 ≤ n ≤ 10⁶"],
...
int main() {
  ...
  },
  {
    ...
    pattern: "Complexity Analysis",
    realWorld:
      "Knowing the worst case prevents developers from using Insertion Sort for large datasets that might be in reverse order.",
...
int main() {
...

  {
    ...
    realWorld:
      "Used when the 'Comparison' operation is very expensive (e.g., comparing large files or complex objects) even though 'Shifting' remains O(n).",
...
<br><br>Return the total number of <strong>comparisons</strong> made during the sort.`,
    examples: [
      {
        input: "arr = [37, 23, 0, 17, 12, 72, 31, 46, 100, 88]",
        output: "Comparisons: 23",
        explain:
          "Binary search reduces the comparison count significantly compared to linear search.",
      },
    ],
    constraints: ["1 ≤ n ≤ 1000"],
...
int insertionSort(vector<int>& arr, int n) {
...

  {
    ...
    pattern: "Recursion",
    realWorld:
      "Used to understand recursive decomposition. It mirrors the 'Stack' behavior where we process smaller sub-arrays before inserting the current element.",
...
<br><br>The base case is when the array size is 1. For size <code>n</code>, first sort <code>n-1</code> elements recursively, then insert the <code>n-th</code> element into its correct position in the sorted part.`,
    examples: [{ input: "arr = [5, 4, 3, 2, 1]", output: "[1, 2, 3, 4, 5]" }],
    constraints: ["1 ≤ n ≤ 500"],
...
int main() {
...

  {
    ...
    pattern: "Incremental Building",
    realWorld:
      "Used in scientific computing where datasets contain absolute changes (delta values) which can be negative.",
...
int main() {
...

  {
    ...
    realWorld:
      "Alphabetizing names in a digital contact book or sorting words in a simple text editor autocomplete list.",
    description: `Given an array of strings, sort them in <strong>lexicographical</strong> (alphabetical) order using Insertion Sort.`,
    examples: [
      {
        input: 'arr = ["banana", "apple", "mango"]',
...
int main() {
...

  {
    ...
    realWorld:
      "Used when you only need to display the 'top' few results (like a leaderboard) without sorting the entire underlying dataset.",
    ...
    examples: [
      {
        ...
        output: "[3, 4, 5, 2, 1]",
        explain: "Only the first three elements are sorted.",
      },
    ],
...
int main() {
...

  {
    ...
    realWorld:
      "Used in teaching tools to show the 'Landing Position' of each element during the sorting process.",
    ...
    examples: [
      {
        input: "arr = [10, 5, 20, 2]",
        output: "[0, 2, 0]",
        explain: "5 moves to idx 0. 20 stays at idx 2. 2 moves to idx 0.",
      },
    ],
    constraints: ["1 ≤ n ≤ 100"],
...
int main() {
...

  {
    ...
    realWorld:
      "Used in adaptive sorting algorithms to decide whether to use a complex $O(N \log N)$ sort or a simple $O(N)$ insertion pass.",
    ...
    examples: [
      {
        input: "arr = [2, 1, 3, 5, 4]",
        output: "true",
        explain: "1 and 2 are swapped (dist 1), 4 and 5 are swapped (dist 1).",
      },
      {
        input: "arr = [3, 2, 1]",
        output: "false",
        explain: "1 is 2 positions away from its correct spot (index 0).",
      },
    ],
    constraints: ["1 ≤ n ≤ 10⁶"],
...
if __name__ == "__main__":
...

  {
    ...
    realWorld:
      "Used in real-time systems where data arrives as a stream of nodes and must be kept in sorted order without the luxury of contiguous memory.",
...
class Solution:
...

  {
    ...
    realWorld:
      "Used to sort objects like 'Students' by their 'Roll Number' or 'Products' by 'Price' while maintaining the original order of items with equal values (Stability).",
...
<br><br>Insertion Sort is <strong>Stable</strong>, meaning if two IDs have the same Value, their relative order should not change.`,
    examples: [
      {
        input: "arr = [{1, 20}, {2, 10}, {3, 20}]",
        output: "[{2, 10}, {1, 20}, {3, 20}]",
        explain: "ID 1 stays before ID 3 as they both have value 20.",
      },
    ],
    constraints: ["1 ≤ n ≤ 1000"],
...
int main() {
...

  {
    ...
    realWorld:
      "Critical for systems using <strong>EEPROM</strong> or <strong>Flash memory</strong> where every 'Write' operation causes physical wear to the hardware.",
    ...
    examples: [
      {
        ...
        output: "[5, 10, 20, 30]",
        explain: "Identify that 5 belongs at index 0, then shift 10, 20, 30.",
      },
    ],
    constraints: ["1 ≤ n ≤ 1000"],
...
void optimizedWritesSort(vector<int>& arr, int n) {
...

  {
    ...
    realWorld:
      "Professional libraries like <strong>Timsort</strong> (Python/Java) and <strong>Introsort</strong> (C++) switch to Insertion Sort when subarray size < 16, as it is faster than QuickSort for tiny data due to low overhead.",
...
<br><br>Sort only the elements between indices <code>low</code> and <code>high</code> (inclusive).`,
    examples: [
      {
        input: "arr = [10, 5, 2, 8, 3, 9, 1], low = 1, high = 4",
        output: "[10, 2, 3, 5, 8, 9, 1]",
        explain: "Only elements from index 1 to 4 are sorted.",
      },
    ],
...
int main() {
...

  {
    ...
    realWorld:
      "Used in data streams where elements are slightly out of order due to network latency, but no element is more than K positions away from its timestamped order.",
    ...
    examples: [
      {
        ...
        output: "[1, 2, 3, 4, 5, 6]",
        explain:
          "1 was at index 2, moved to 0 (dist 2). 4 was at index 5, moved to 3 (dist 2).",
      },
    ],
